...
public class TexturePipeline {
    ...
    private int getTextureGlId(GpuTexture gpuTexture) {
        // Fast path: the GL backend always returns a GlTexture, which exposes a
        // public getGlId(). Avoids any reflection entirely.
        if (gpuTexture instanceof GlTexture glTexture) {
            return glTexture.getGlId();
        }

        // Fallback (non-GL backend / unexpected impl): resolve via reflection,
        // cached per class. Preserves the previous behaviour exactly.
        ...
        // Fallback: scan all int fields for a plausible GL id (also used when no
        // field was cached, e.g. the named field couldn't be resolved).
        try {
