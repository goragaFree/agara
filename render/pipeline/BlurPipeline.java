...
public class BlurPipeline {
    ...
    // Batch mode: copy the framebuffer once and let every blur in the batch sample
    // that single snapshot, instead of copying the whole framebuffer per call.
    ...
    public void drawBlur(float x, float y, float width, float height,
        ...
        // Outside a batch: always re-copy. Inside a batch: copy once on the first
        // blur, then every following blur samples that same snapshot.
        if (!batchMode || !copyValid) {
