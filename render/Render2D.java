...
public class Render2D {
    ...
    /**
     * Live frosted-glass blur for UI panels. Disabled: the snapshot is captured in
     * {@code afterGuiRender}, where the main framebuffer is not reliably the freshly
     * rendered world, so it freezes on a stale frame (panels show an old scene that
     * only updates occasionally). Until the capture is moved into the world-render
     * pass and verified in-game, {@link #glass} falls back to a clean translucent tint.
     */
    public static boolean USE_UI_BLUR = true;
    ...
    /**
     * Flushes any pending batched rectangles. Must be called before any non-rect
     * draw (outline/texture/text/blur), before scissor changes, and at frame end,
     * so batched rects keep their original z-order. Null-guarded for early init.
     */
    public static void flushRects() {
        Initialization init = Initialization.getInstance();
        if (init == null) return;
        var manager = init.getManager();
        if (manager == null) return;
        var core = manager.getRenderCore();
        if (core == null) return;
        var pipeline = core.getRectPipeline();
        if (pipeline != null) {
            pipeline.flush();
        }
        // Commit any batched text at this z-boundary too: an immediate draw
        // (texture/outline/blur) calls flushRects() right before it draws, so the
        // deferred text from before that point gets committed underneath it
        // instead of ending up on top. No-op when text isn't being batched.
        var fontRenderer = core.getFontRenderer();
    ...
    /**
     * Frosted-glass rounded rectangle: samples the once-per-frame Kawase blur of
     * the screen behind it and tints it. {@code tint} rgb = tint color, alpha =
     * tint strength (how much it darkens/colors the blur). Falls back to a plain
     * tinted rect if the blur isn't available yet.
     */
    public static void glass(float x, float y, float width, float height, float radius, int tint) {
        glass(x, y, width, height, radius, radius, radius, radius, tint, 1f);
    }
    ...
    public static void glass(float x, float y, float width, float height,
        ...
        if (blurred != null && core != null && core.getGlassRectPipeline() != null) {
            ...
            core.getGlassRectPipeline().draw(x, y, width, height, radii, tint, opacity,
                    Theme.GLASS_DISTORTION,
                    Theme.GLASS_STRENGTH, blurred);
        } else {
            int a = Math.max(0, Math.min(255, (int) (((tint >> 24) & 0xFF) * opacity)));
            int fallback = (a << 24) | (tint & 0xFFFFFF);
            rect(x, y, width, height, fallback, topLeft, topRight, bottomRight, bottomLeft);
        }
    }
