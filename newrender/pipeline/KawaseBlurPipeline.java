...
public class KawaseBlurPipeline {
    ...
    private static final RenderPipeline DOWN_PIPELINE = RenderPipelines.register(
            RenderPipeline.builder(RenderPipelines.TRANSFORMS_AND_PROJECTION_SNIPPET)
                    ...
                    // No blend: each pass is a pure resample that overwrites the whole
                    // target. Blending would mix with the previous frame's chain contents
                    // (the targets persist and are never cleared) — ghosting risk.
    ...
    public GpuTextureView blur(GpuTexture sourceTexture, GpuTextureView sourceView, int width, int height, int iterations, float offset) {
        ...
        // The chain ends at half input resolution. All consumers sample by UV
        // with a LINEAR sampler, so upscaling happens for free at sample time —
        // no full-res final pass or full-res texture needed.
        return upTextureViews[0];
    }
