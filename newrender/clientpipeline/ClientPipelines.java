...
public class ClientPipelines {
    ...
    // Радиальное свечение из вершин (без текстуры): аддитивно, без depth-теста.
    // Центр яркий, края прозрачные -> плавное затухание наружу.
    public static final RenderPipeline GLOW_FILL_PIPELINE = RenderPipelines.register(
            RenderPipeline.builder(TRANSFORMS_AND_PROJECTION_SNIPPET)
                    .withLocation("pipeline/crystal_glow")
                    .withVertexShader("core/position_color")
                    .withFragmentShader("core/position_color")
    ...
    public static final RenderLayer GLOW_FILL = RenderLayer.of(
            "crystal_glow",
            RenderSetup.builder(GLOW_FILL_PIPELINE)
                    .translucent()
                    .expectedBufferSize(4096)
                    .build()
    );
    ...
    // Слои для Render3D: линии/квады без depth-теста (видны сквозь стены).
    public static final RenderPipeline THROUGH_WALLS_LINES_PIPELINE = RenderPipelines.register(
            RenderPipeline.builder(RenderPipelines.POSITION_COLOR_SNIPPET)
                    .withLocation(Identifier.of("rich", "through_walls_lines"))
                    .withVertexFormat(VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.DEBUG_LINES)
                    .withCull(false)
                    .withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST)
                    .withDepthWrite(false)
    ...
    public static final RenderLayer THROUGH_WALLS_LINES = RenderLayer.of(
            "through_walls_lines",
            RenderSetup.builder(THROUGH_WALLS_LINES_PIPELINE)
                    .translucent()
                    .expectedBufferSize(4096)
                    .build()
    );

    public static final RenderPipeline THROUGH_WALLS_QUADS_PIPELINE = RenderPipelines.register(
            RenderPipeline.builder(RenderPipelines.POSITION_COLOR_SNIPPET)
                    .withLocation(Identifier.of("rich", "through_walls_quads"))
                    .withVertexFormat(VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.QUADS)
                    .withCull(false)
                    .withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST)
                    .withDepthWrite(false)
    ...
    public static final RenderLayer THROUGH_WALLS_QUADS = RenderLayer.of(
            "through_walls_quads",
            RenderSetup.builder(THROUGH_WALLS_QUADS_PIPELINE)
                    .translucent()
                    .expectedBufferSize(8192)
                    .build()
    );
