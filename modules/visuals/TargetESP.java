...
public class TargetESP extends BaseModule {
   ...
   private void drawCrystals(MatrixStack ms, LivingEntity target) {
      ...
      for (int i = 0; i < 360; i += 20) {
         ...
         DrawUtility.drawImage(ms, buffer, -bigSize / 2.0F, -bigSize / 2.0F, 0.0, bigSize, bigSize, color.withAlpha(255.0F * this.animation.getValue() * 0.2F));
         ms.pop();
      }
      ...
      for (int i = 0; i < 360; i += step) {
         ...
         if (wormCD > 0) {
            ...
            if (wormTick > 50) {
               ...
               DrawUtility.drawImage(
                  ms, builder, -size / 2.0F, -size / 2.0F, -size / 2.0F, size, size, color.withAlpha(color.getAlpha() * this.animation.getValue())
               );
               ms.pop();
            }
         }
