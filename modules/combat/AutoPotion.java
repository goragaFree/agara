...
public class AutoPotion extends BaseModule {
      ...
      this.cerka = new AutoPotion.PotionValue(
            ...
            () -> {
               ...
               ItemSlot health = slotsToSearch.findItem(
                  stack -> PotionUtility.hasEffect(stack, StatusEffects.INSTANT_HEALTH) && !mc.player.getItemCooldownManager().isCoolingDown(stack)
               );
               ItemSlot golden = slotsToSearch.findItem(
                  stack -> stack.getItem() == Items.ENCHANTED_GOLDEN_APPLE && !mc.player.getItemCooldownManager().isCoolingDown(stack)
               );
               boolean near = false;
               ...
               return mc.player.getHealth() + mc.player.getAbsorptionAmount() <= this.cerkaHealth.getCurrentValue()
                  && golden == null
                  && health == null
                  && near;
            }
   ...
   private void usePotion() {
      ...
      ((ClientPlayerInteractionManagerAccessor)mc.interactionManager)
         .invokeSendSequencedPacket(mc.world, sequence -> new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, sequence, rotation.getYaw(), rotation.getPitch()));
   }
      ...
      return stack -> {
         if (!stack.isEmpty() && stack.getItem() instanceof SplashPotionItem) {
            ...
            return !this.don.isSelected() && type != StatusEffects.WEAKNESS
               ? effects.size() == 1 && ((StatusEffectInstance)effects.getFirst()).getEffectType() == type
