...
public class Auction extends BaseModule {
   ...
   private final EventListener<ScreenRenderEvent> onScreen = event -> {
      if (!this.pageItems.isEmpty() && mc.currentScreen instanceof HandledScreen screen && this.isAuction(screen.getTitle().getString())) {
            ...
            for (Auction.AuctionItem item : this.pageItems) {
               if (!(item.effectivePrice > this.averageEffectivePrice)) {
                  ...
                  if (slotToHighlight != null) {
                     ...
                     event.getContext()
                        .drawRoundedRect(
                           x, y, 16.0F, 16.0F, BorderRadius.all(1.0F), new VerticalGradient(color.withAlpha(0.0F), color.withAlpha(0.8F * color.getAlpha()))
                        );
                  }
               }
            }
