...
public class DynamicIsland extends HudElement implements IMinecraft, IScaledResolution {
   ...
   protected void renderComponent(UIContext context) {
      ...
      context.drawClientRect(this.x, this.y, this.width, this.height, 1.0F, this.dragAnim.getValue(), 2.0F + 5.0F * this.extendingAnim.getValue());
      ...
      if (!this.extended
         ...
         && GuiUtility.isHovered(this.x, this.y, this.width, this.height, GuiUtility.getMouse().getX(), GuiUtility.getMouse().getY())) {
   ...
   public boolean handleClick(float mouseX, float mouseY, int button) {
      ...
      float y = 7.0F;
      if (this.extended) {
         if (!GuiUtility.isHovered(x, y, this.size.width, this.size.height, mouseX, mouseY)) {
            this.extended = false;
         } else {
         ...
         return true;
      } else if (GuiUtility.isHovered(x, y, this.size.width, this.size.height, mouseX, mouseY) && this.active() instanceof ExtandableStatus) {
         this.extended = true;
         return true;
      } else {
         return false;
   ...
   public IslandStatus active() {
      return (IslandStatus)this.statuses().getLast();
   }
