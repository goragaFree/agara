...
void main() {
    ...
    // Isotropic SDF anti-aliasing. Use the true screen-space gradient length of
    // the distance field: it stays ~1px all the way around, including the
    // rounded corners. (fwidth(dist) = |dFdx|+|dFdy| inflates ~1.41x on the 45
    // degree diagonal and softens the corner; max(fwidth(center.x/y)) is a
    // square band that reads stepped on the curve.) smoothstep over a ~1.5px
    // band then gives a clean, smooth edge instead of a hard stair-stepped one.
    float aa = max(length(vec2(dFdx(dist), dFdy(dist))), 0.0001);
    float alpha;
    if (softness > aa) {
        alpha = 1.0 - smoothstep(-softness, softness, dist);
    } else {
        alpha = 1.0 - smoothstep(-aa * 0.75, aa * 0.75, dist);
    }

    // Режим тени (cutInner): гасим внутренность фигуры, оставляя только внешнюю
    // растушёвку — тень под ПОЛУПРОЗРАЧНОЙ панелью не просвечивает сквозь неё и
    // не меняет её плотность. Спад начинается чуть ВНУТРИ кромки (~1.5px), чтобы
    // под AA-краем панели тень ещё была — иначе по границе читался бы светлый
    // зазор. Бонус: внутренние пиксели уходят в discard — тень дешевле.
    if (cutInner > 0.5) {
        alpha *= smoothstep(-1.5, -0.25, dist);
    }
