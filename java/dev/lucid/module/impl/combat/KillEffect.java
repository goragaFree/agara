...
public class KillEffect extends Module {
	...
	private static final String MODE_CHOP = "Рубящий";
	private static final String MODE_SWEEP = "Круговой";

	/** Колющий: один меч уводит за собой силуэты, и весь веер бьёт сверху вниз. */
	private static final String MODE_STAB = "Колющий";
	...
	/**
	 * С какой доли эффекта начинает таять тело в рубящем и круговом.
	 *
	 * <p>У этих двух режимов удар идёт к концу эффекта, а не на первой секунде, так
	 * что раньше тело гасить нельзя: мечи ещё только заносятся.</p>
	 */
	private static final float CORPSE_FADE_FROM = 0.9F;
	...
	// --- фазы анимации в долях от общей длительности ---

	private static final float APPEAR_END = 0.18F;
	private static final float WINDUP_END = 0.46F;
	private static final float SLASH_END = 0.74F;

	/** Сдвиг соседнего меча по времени: они бьют по очереди, а не хором. */
	private static final float STAGGER = 0.10F;

	/** Замах: клинок отведён назад, от цели. */
	private static final float WINDUP_ANGLE = 48.0F;

	/** Конец удара: клинок прошёл цель и смотрит вниз-внутрь. */
	private static final float SLASH_ANGLE = -105.0F;
	...
	private final ModeSetting animation = this.addSetting(new ModeSetting(
			"animation", "Анимация", "Как мечи добивают цель",
			MODE_CHOP, List.of(MODE_CHOP, MODE_SWEEP, MODE_STAB)));
	...
	private float corpseAlpha(Effect effect, long now) {
		if (this.animation.is(MODE_STAB)) {
		...
		float fade = (effect.progress(now) - CORPSE_FADE_FROM) / (1.0F - CORPSE_FADE_FROM);

		return 1.0F - MathUtil.smoothStep(MathUtil.clamp01(fade));
	}
	...
	private void tickEffects(ClientLevel currentLevel) {
		...
		for (Effect effect : this.effects) {
			...
			// Момент удара у режимов разный: рубящий и круговой считают его от своей
			// фазы, колющий — по секундам, потому что и бьёт он по секундам.
			boolean stab = this.animation.is(MODE_STAB);
			...
			for (int index = 0; index < effect.bursts.length; index++) {
				...
				if (!effect.appeared[index] && (stab
						? stabTime(effect, index, now) >= stabBirth(index, effect.bursts.length)
						: swordProgress(progress, index, effect.bursts.length) > 0.0F)) {
					effect.appeared[index] = true;
					this.playAppear(currentLevel, effect, index);
				}

				boolean landed = stab
						? stabTime(effect, index, now) >= stabImpact
						: swordProgress(progress, index, effect.bursts.length) >= SLASH_END;
	...
	private void extract(Render3D.Frame frame) {
		...
		long now = System.nanoTime();
		boolean sweep = this.animation.is(MODE_SWEEP);
		boolean stab = this.animation.is(MODE_STAB);
		...
		for (Effect effect : this.effects) {
			...
			for (int index = 0; index < count; index++) {
				...
				if (stab) {
					...
					slot = faceYaw - effect.baseYaw
							+ stabSlot(index, count) * effect.fanSpread
							+ effect.fanTwist;
				} else {
					float local = swordProgress(progress, index, count);

					// Меч, чья очередь ещё не пришла, не должен висеть в воздухе с нулевой фазой.
					if (local <= 0.0F) {
						continue;
					}

					if (sweep) {
						sweepPose(local, this.pose);
					} else {
						chopPose(local, this.pose);
					}
				...
				// Насколько меч уже сел в тело: 0 — ещё идёт по кольцу, 1 — воткнут.
				float settle = 0.0F;

				if (stab) {
					float lead = STAB_TRAVEL_SEC + STAB_RAISE_SEC;
					settle = MathUtil.clamp01((stabTime(effect, index, now) - lead) / STAB_DROP_SEC);
				}
	...
	private void drawSword(
		...
		if (this.animation.is(MODE_STAB)) {
			...
			// поэтому и кольцо, и высота обязаны считаться от них. Пока и то и другое
			// шло от одних ползунков, в мелкую цель мечи втыкались по краю — в ноги
			// или мимо, а рукояти при этом разъезжались веером.
			...
			ringRadius = this.radius.get() * this.pose.radius * effect.jitterRadius[index];
			gripY = effect.y + effect.height * this.height.get() + this.pose.lift * 0.45 * blade
					+ effect.jitterLift[index] * blade;
		}
	...
	/**
	 * Своя фаза меча внутри общего времени эффекта.
	 *
	 * <p>Мечи бьют по очереди, поэтому каждый следующий стартует позже, а его собственная
	 * анимация сжимается — чтобы последний успел догореть до конца эффекта.</p>
	 */
	private static float swordProgress(float progress, int index, int count) {
		float shift = STAGGER * index;
		float span = 1.0F - STAGGER * (count - 1);

		if (span < 0.2F) {
			span = 0.2F;
		}

		return MathUtil.clamp01((progress - shift) / span);
	}

	/** Рубящий: появился сбоку, отвёл клинок, уронил его на цель, растворился. */
	private static void chopPose(float p, Pose out) {
		out.grow = 1.0F;

		out.yaw = 0.0F;
		out.trail = 0.0F;

		// Клинок идёт по дуге замаха, поэтому и шлейф отводится только по замаху.
		out.trailSwing = 15.0F;
		out.trailYaw = 0.0F;
		out.trailLift = 0.0F;

		// Ширина лезвия уже лежит в плоскости замаха — доворачивать нечего.
		out.roll = 0.0F;

		if (p < APPEAR_END) {
			float t = MathUtil.smoothStep(p / APPEAR_END);
			out.alpha = t;
			out.swing = MathUtil.lerp(12.0F, WINDUP_ANGLE * 0.6F, t);
			out.radius = MathUtil.lerp(1.8F, 1.15F, t);
			out.lift = MathUtil.lerp(0.4F, 1.0F, t);
			return;
		}

		if (p < WINDUP_END) {
			float t = MathUtil.smoothStep((p - APPEAR_END) / (WINDUP_END - APPEAR_END));
			out.alpha = 1.0F;
			out.swing = MathUtil.lerp(WINDUP_ANGLE * 0.6F, WINDUP_ANGLE, t);
			out.radius = MathUtil.lerp(1.15F, 1.05F, t);
			out.lift = MathUtil.lerp(1.0F, 1.12F, t);
			return;
		}

		if (p < SLASH_END) {
			float raw = (p - WINDUP_END) / (SLASH_END - WINDUP_END);

			// Квадрат, а не smoothStep: удар должен начаться медленно и хлестнуть в конце,
			// а smoothStep гасит скорость там, где как раз нужен щелчок.
			float t = raw * raw;

			out.alpha = 1.0F;
			out.swing = MathUtil.lerp(WINDUP_ANGLE, SLASH_ANGLE, t);
			out.radius = MathUtil.lerp(1.05F, 0.3F, t);
			out.lift = MathUtil.lerp(1.12F, 0.5F, t);
			out.trail = raw;
			return;
		}

		float t = MathUtil.smoothStep((p - SLASH_END) / (1.0F - SLASH_END));
		out.alpha = 1.0F - t;
		out.swing = MathUtil.lerp(SLASH_ANGLE, SLASH_ANGLE - 10.0F, t);
		out.radius = MathUtil.lerp(0.3F, 0.18F, t);
		out.lift = MathUtil.lerp(0.5F, 0.42F, t);
		out.trail = 1.0F - t;
	}

	/** Круговой: клинки ложатся плашмя и проносятся вокруг цели по кругу. */
	private static void sweepPose(float p, Pose out) {
		out.grow = 1.0F;
		...
		// Здесь меч идёт по касательной, а не в плоскости замаха, поэтому лезвие надо
		// положить плашмя: коса под ноги, а не топор сверху.
		out.roll = 90.0F;

		if (p < APPEAR_END) {
			float t = MathUtil.smoothStep(p / APPEAR_END);
			out.alpha = t;
			out.swing = MathUtil.lerp(-15.0F, -78.0F, t);
			out.radius = MathUtil.lerp(1.9F, 1.15F, t);
			out.lift = 1.0F;
			out.yaw = MathUtil.lerp(-45.0F, 0.0F, t);
			return;
		}
		...
		if (p < WINDUP_END) {
			float t = MathUtil.smoothStep((p - APPEAR_END) / (WINDUP_END - APPEAR_END));
			out.alpha = 1.0F;
			out.swing = MathUtil.lerp(-78.0F, -88.0F, t);
			out.radius = MathUtil.lerp(1.15F, 1.1F, t);
			out.lift = MathUtil.lerp(1.0F, 1.05F, t);
			out.yaw = MathUtil.lerp(0.0F, 25.0F, t);
			return;
		}
		...
		if (p < SLASH_END) {
			...
			out.alpha = 1.0F;
			out.swing = MathUtil.lerp(-88.0F, -96.0F, t);
			out.radius = MathUtil.lerp(1.1F, 0.4F, t);
			out.lift = MathUtil.lerp(1.05F, 0.75F, t);
			out.yaw = MathUtil.lerp(25.0F, 300.0F, t);
			out.trail = raw;
			return;
		}
		...
		out.alpha = 1.0F - t;
		out.swing = -96.0F;
		out.radius = MathUtil.lerp(0.4F, 0.25F, t);
		out.lift = MathUtil.lerp(0.75F, 0.65F, t);
		out.yaw = MathUtil.lerp(300.0F, 340.0F, t);
		out.trail = 1.0F - t;
	}
	 ...
	 *
	 * <p>Поза зависит не только от фазы, но и от номера меча, и в этом всё отличие от
	 * двух других режимов. Там у каждого клинка своя фаза со сдвигом
	 * ({@link #swordProgress}), здесь отсчёт общий на весь залп: мечи обязаны ударить
	 * разом, а до удара — стоять там, где их оставил ведущий.</p>
	 *
	...
	private static void stabPose(float seconds, float total, int index, int count, Pose out) {
		...
		if (seconds < STAB_TRAVEL_SEC + STAB_RAISE_SEC + STAB_DROP_SEC) {
			...
			// Квадрат, как и в рубящем: удар должен разгоняться к концу, а не тормозить.
			float t = raw * raw;
	...
	private static final class Pose {
		...
		/** Доворот вокруг цели в градусах — им живёт круговой режим. */
		float yaw;

		/**
		 * Крен вокруг своей же оси клинка в градусах.
		 *
		 * <p>Режимы бьют в разных направлениях: рубящий ведёт острие сверху вниз, а круговой —
		 * по касательной вокруг цели. Лезвие должно смотреть вперёд по ходу удара в обоих
		 * случаях, а это разные крены с разницей в 90°.</p>
		 */
		float roll;
		 ...
		 *
		 * <p>Шлейф должен лежать на уже пройденном пути, а путь у режимов разный: рубящий и
		 * круговой идут углом замаха, колющий сначала вбок по облёту, а потом строго вниз.
		 * Одним смещением по замаху второе и третье не покажешь.</p>
		 */
		float trailSwing;
